<?php 
if($_GET['hiddenvalue']==1  && isset($_GET['category']) && !empty($_GET['category']))
{ 

$cmg_registration=$conn->prepare("SELECT * FROM cmg_registration a , cmg_basic_inf0_ti  b WHERE a.status=1 AND  a.UMID=b.UMID   AND b.premium_course RLIKE '[[:<:]]" . $_GET['category']. "[[:>:]]' and  b.city='".$_SESSION['currentcity']."' AND a.status=1 AND a.email_verification=1 LIMIT 0,5  ");
}
else {
	$cmg_registration=$conn->prepare("SELECT * FROM cmg_registration a , cmg_basic_inf0_ti  b WHERE a.status=1 AND  a.UMID=b.UMID  AND b.primum_city='".$_SESSION['currentcity']."' AND a.status=1 AND a.email_verification=1 LIMIT 0,5  ");
}
