<?php $cmg_registration=$conn->prepare("SELECT * FROM cmg_registration a , cmg_basic_inf0_ti  b WHERE a.status=1 AND  a.UMID=b.UMID AND b.categorypost RLIKE '[[:<:]]" . $_GET['Post'] . "[[:>:]]'  AND b.city='".$_SESSION['currentcity']."' AND a.status=1 AND a.email_verification=1 LIMIT 0,5  ");
