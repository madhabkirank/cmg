...
$DataMessage=$conn->prepare("SELECT * FROM (SELECT * FROM cmg_message WHERE `UMID_from`='".$_SESSION['s-UMID']."' AND category='Followers' AND from_message='trainer' ORDER BY `id` DESC) cmg_message GROUP BY `UMID_to` ORDER BY `id` DESC ");
...
$unreadmessage  = num_rows('cmg_message',array('UMID_to' => $DataMessageFE['UMID_to'],'UMID_from' => $_SESSION['s-UMID'],'from_message'=>'trainer','new_status'=>0,'category' => 'Followers'));
$unreadmessageNEW= num_rows('cmg_message',array('UMID_to' => $DataMessageFE['UMID_to'],'UMID_from' => $_SESSION['s-UMID'],'from_message'=>'trainer','category' => 'Followers'));			 
 ...
 ?>
				...
				<div class="stud_msg">
					...
					<?=ucfirst(nl2br($DataMessageFE['message_text']));?>
					   <?php if($unreadmessage>0){?>  <a href="User-Message-Chat.php?Message_ID=<?=$DataMessageFE['UMID_to'];?>#Sprofile"><div class="reply"> &nbsp; Reply</div></a> &nbsp; <a href="User-Message-Chat.php?Message_ID=<?=$DataMessageFE['UMID_to'];?>#Sprofile"><div class="reply" style="background:#DF6C02">&nbsp;<?=$unreadmessage;?> - Unread</div></a> <?php } else { ?> 
					     <a href="User-Message-Chat.php?Message_ID=<?=$DataMessageFE['UMID_to'];?>#Sprofile"><div class="reply"> &nbsp; <?=$unreadmessageNEW;?> - Reply</div></a> &nbsp;
						 <?php } ?>
					</div>
