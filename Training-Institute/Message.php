...
$DataMessage=$conn->prepare("SELECT * FROM (SELECT * FROM cmg_message WHERE `UMID_to`='".$_SESSION['UMID']."' AND category='General' AND from_message='student' ORDER BY `id` DESC) cmg_message GROUP BY `UMID_from` ORDER BY `id` DESC ");
...
$unreadmessage  = num_rows('cmg_message',array('UMID_to' => $_SESSION['UMID'],'UMID_from' => $DataMessageFE['UMID_from'],'from_message'=>'student','new_status'=>0,'category' => 'General'));

$unreadmessagenew  = num_rows('cmg_message',array('UMID_to' => $_SESSION['UMID'],'UMID_from' => $DataMessageFE['UMID_from'],'from_message'=>'student','category' => 'General'));
 ...
 ?>
				...
				<div class="stud_msg">
					...
					<?=ucfirst(nl2br($DataMessageFE['message_text']));?>
					   ...
					   <?php if($unreadmessage>0){?>  <a href="User-Message-Chat-general.php?Message_ID=<?=$DataMessageFE['UMID_from'];?>#Profile"><div class="reply"> &nbsp; Reply</div></a> &nbsp; <a href="User-Message-Chat-general.php?Message_ID=<?=$DataMessageFE['UMID_from'];?>#Profile"><div class="reply" style="background:#DF6C02">&nbsp;<?=$unreadmessage;?> - Unread</div></a> <?php } else { ?> 
					    <a href="User-Message-Chat-general.php?Message_ID=<?=$DataMessageFE['UMID_from'];?>#Profile"><div class="reply"> &nbsp; <?=$unreadmessagenew;?> - Reply</div></a> &nbsp;
						<?php } ?>
					</div>
